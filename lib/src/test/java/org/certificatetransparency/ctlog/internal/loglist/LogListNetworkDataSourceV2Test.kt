...
class LogListNetworkDataSourceV2Test {
    ...
    fun verifiesSignature() = runBlocking {
        ...
        assertEquals("aPaY+B9kgr46jO65KB1M/HFRXWeT1ETRCmesu09P+8Q=", Base64.toBase64String(result[0].id))
    }
    ...
    @Test
    fun validUntilNullWhenNotFrozenOrRetired() = runBlocking {
        // given we have a valid json file and signature
        expectInterceptor("http://ctlog/log_list.json", json)
        val keyPair = generateKeyPair()
        val signature = calculateSignature(keyPair.private, json.toByteArray())
    ...
    @Test
    fun validUntilSetFromFrozen() = runBlocking {
        // given we have a valid json file and signature
        expectInterceptor("http://ctlog/log_list.json", json)
        val keyPair = generateKeyPair()
        val signature = calculateSignature(keyPair.private, json.toByteArray())
    ...
    @Test
    fun validUntilSetFromRetired() = runBlocking {
        // given we have a valid json file and signature
        expectInterceptor("http://ctlog/log_list.json", json)
        val keyPair = generateKeyPair()
        val signature = calculateSignature(keyPair.private, json.toByteArray())
