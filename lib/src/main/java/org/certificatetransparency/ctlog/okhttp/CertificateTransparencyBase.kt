...
open class CertificateTransparencyBase(
    ...
    private fun buildLogSignatureVerifiers() {
        ...
        val json = LOG_LIST_JSON2//TestData.file(TEST_LOG_LIST_JSON).readText()
        ...
        for (trustedLogKey in trustedLogKeys) {
            hasher.reset()
            val keyBytes = org.bouncycastle.util.encoders.Base64.decode(trustedLogKey)
            val logId = org.bouncycastle.util.encoders.Base64.toBase64String(hasher.digest(keyBytes))
...
private val LOG_LIST_JSON2: String = """
            {
 "logs": [
  {
